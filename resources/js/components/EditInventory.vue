        ...
        <div>
            <form @submit.prevent="getFormValues">
                ...
                <div v-if="editShow">
                    ...
                    <button type="button" @click="erase" class="btn btn-danger">
                        Delete
                    </button>
                </div>
            </form>
...
import { bus } from "../app";
import { getInventories, getCategories } from "./inventory";
export default {
    created() {
        bus.$on("newInventory", () => {
            this.getInventories();
            ...
            const result = await axios
                ...
                .catch(err => {
                    ...
                    if (err.response.data === "Inventory already exists") {
                        ...
                        bus.$emit(
                            "errorAlert",
                            err.response.data.errors.all_properties[0]
                        );
                        ...
                        bus.$emit(
                            "errorAlert",
                            err.response.data.errors.inventory[0]
                        );
                    } else {
                        console.log(err);
                    }
